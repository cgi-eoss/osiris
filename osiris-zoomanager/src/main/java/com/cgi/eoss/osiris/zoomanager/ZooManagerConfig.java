...
public class ZooManagerConfig {
    ...
    @Bean
    public String javacClasspath(@Value("${osiris.zoomanager.javac.classpath}") String javacClasspath) {
        return javacClasspath;
    }

    @Bean
    public Path zcfgBasePath(@Value("${osiris.zoomanager.zcfg.path:/var/www/cgi-bin}") String zcfgBasePath) {
        return Paths.get(zcfgBasePath);
    }

    @Bean
    public Path zooServicesStubJar(@Value("${osiris.zoomanager.stub.jarFile:/var/www/cgi-bin/jars/osiris-services.jar}") String zooServicesStubJar) {
