...
import com.cgi.eoss.osiris.rpc.worker.PrepareDockerImageResponse;
import com.cgi.eoss.osiris.rpc.worker.StopContainerResponse;
import com.cgi.eoss.osiris.worker.DockerRegistryConfig;
import com.cgi.eoss.osiris.worker.docker.DockerClientFactory;
import com.cgi.eoss.osiris.worker.docker.Log4jContainerCallback;
...
/**
 * <p>Service for executing OSIRIS (WPS) services inside Docker containers.</p>
 */
@GRpcService
@Log4j2
public class OsirisWorker extends OsirisWorkerGrpc.OsirisWorkerImplBase {
    ...
    @Autowired
    public OsirisWorker(OsirisWorkerNodeManager nodeManager, JobEnvironmentService jobEnvironmentService,
    ...
    public void allocateMinNodes() {
        int currentNodes = nodeManager.getCurrentNodes(OsirisWorkerNodeManager.POOLED_WORKER_TAG).size();
        if (currentNodes < minWorkerNodes) {
            try {
                nodeManager.provisionNodes(minWorkerNodes - currentNodes, OsirisWorkerNodeManager.POOLED_WORKER_TAG, jobEnvironmentService.getBaseDir());
    ...
    public void prepareInputs(JobInputs request, StreamObserver<JobEnvironment> responseObserver) {
        try (CloseableThreadContext.Instance ctc = getJobLoggingContext(request.getJob())) {
            ...
            try {
                ...
                com.cgi.eoss.osiris.worker.worker.JobEnvironment jobEnv = jobEnvironmentService.createEnvironment(request.getJob().getId(), inputs);
   ...
   public void prepareDockerImage(DockerImageConfig request, StreamObserver<PrepareDockerImageResponse> responseObserver) {
    ...
    @Override
    public void cleanUp(com.cgi.eoss.osiris.rpc.Job request,
            io.grpc.stub.StreamObserver<com.cgi.eoss.osiris.rpc.worker.CleanUpResponse> responseObserver) {
    ...
    private static CloseableThreadContext.Instance getJobLoggingContext(Job job) {
        return CloseableThreadContext.push("OSIRIS Worker")
                .put("zooId", job.getId())
