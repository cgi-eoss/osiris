...
/**
 * <p>Provides common utility-style methods for interacting with the OSIRIS security context.</p>
 */
@Component
@Log4j2
public class OsirisSecurityService {
    public static final Authentication PUBLIC_AUTHENTICATION = new PreAuthenticatedAuthenticationToken("PUBLIC", "N/A", ImmutableList.of(OsirisPermission.PUBLIC));
    ...
    @Autowired
    public OsirisSecurityService(MutableAclService aclService, AclPermissionEvaluator aclPermissionEvaluator, UserDataService userDataService, PublishingRequestDataService publishingRequestDataService) {
    ...
    public void updateOwnerWithCurrentUser(OsirisEntityWithOwner entity) {
    ...
    public void publish(ObjectIdentity objectIdentity) {
        ...
        Sid publicSid = new GrantedAuthoritySid(OsirisPermission.PUBLIC);
    ...
    public void unpublish(Class<?> objectClass, Long identifier) {
        ...
        IntStream.range(0, aceCount).map(i -> aceCount - i - 1)
                .filter(i -> acl.getEntries().get(i).getSid().equals(publicSid) && OsirisPermission.READ.getAclPermissions().contains(acl.getEntries().get(i).getPermission()))
     ...
     * @return Whether the object is published or not, as well as the {@link OsirisPermission} corresponding to the
     * current access level.
     */
    public OsirisAccess getCurrentAccess(Class<?> objectClass, Long identifier) {
        Authentication authentication = getCurrentAuthentication();
        ObjectIdentity objectIdentity = new ObjectIdentityImpl(objectClass, identifier);

        return OsirisAccess.builder()
    ...
    private OsirisPermission getCurrentPermission(Authentication authentication, ObjectIdentity objectIdentity) {
        if (isSuperUser()) {
            return OsirisPermission.SUPERUSER;
        } else if (hasOsirisPermission(authentication, OsirisPermission.ADMIN, objectIdentity)) {
            return OsirisPermission.ADMIN;
        } else if (hasOsirisPermission(authentication, OsirisPermission.WRITE, objectIdentity)) {
            return OsirisPermission.WRITE;
        } else if (hasOsirisPermission(authentication, OsirisPermission.READ, objectIdentity) || isPublic(objectIdentity)) {
            return OsirisPermission.READ;
        } else {
            return null;
        }
    }
    ...
    /**
     * <p>Verify that the OsirisPermission.READ permission is granted on the given object to the static PUBLIC
    ...
    public boolean isPublic(ObjectIdentity objectIdentity) {
        return hasOsirisPermission(PUBLIC_AUTHENTICATION, OsirisPermission.READ, objectIdentity);
    }
     ...
     */
    public boolean hasUserPermission(User user, OsirisPermission permission, Class<?> objectClass, Long identifier) {
     ...
     * @return True if the object has the requested permission for the requested user.
     */
    public boolean hasUserPermission(User user, OsirisPermission permission, ObjectIdentity objectIdentity) {
        Authentication authentication = getAuthentication(user);
        return isSuperUser(authentication) || hasOsirisPermission(authentication, permission, objectIdentity);
    }
    ...
    private boolean hasOsirisPermission(Authentication authentication, OsirisPermission permission, ObjectIdentity objectIdentity) {
    ...
    public boolean isReadableByCurrentUser(Class<?> objectClass, Long objectId) {
        return isSuperUser() || hasOsirisPermission(getCurrentAuthentication(), OsirisPermission.READ, new ObjectIdentityImpl(objectClass, objectId));
    }
}
