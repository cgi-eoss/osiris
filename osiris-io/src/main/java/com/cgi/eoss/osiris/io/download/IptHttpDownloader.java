...
public class IptHttpDownloader implements Downloader {
    ...
    @Autowired
    IptHttpDownloader(OkHttpClient okHttpClient, OsirisServerClient osirisServerClient, DownloaderFacade downloaderFacade, IptHttpDownloaderProperties properties) {
	...
	private Path iptDownload(Path targetDir, URI uri) throws IOException {
        ...
        Credentials credentials = osirisServerClient.credentialsServiceBlockingStub().getCredentials(
    ...
    private static final class IptHttpDownloaderProperties {
        @Value("${osiris.worker.downloader.ipt.searchUrl:https://finder.eocloud.eu/resto/}")
        private String iptSearchUrl;
        @Value("${osiris.worker.downloader.ipt.downloadBaseUrl:https://static.eocloud.eu/v1/AUTH_8f07679eeb0a43b19b33669a4c888c45}")
        private String iptDownloadUrl;
        @Value("${osiris.worker.downloader.ipt.authEndpoint:https://finder.eocloud.eu/resto/api/authidentity}")
        private String authEndpoint;
        @Value("${osiris.worker.downloader.ipt.authDomain:__secret__}")
