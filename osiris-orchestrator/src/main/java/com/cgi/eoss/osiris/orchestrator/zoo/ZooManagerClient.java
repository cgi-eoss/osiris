package com.cgi.eoss.osiris.orchestrator.zoo;

import com.cgi.eoss.osiris.model.OsirisService;
import com.cgi.eoss.osiris.rpc.WpsServiceDescriptor;
import com.cgi.eoss.osiris.rpc.WpsServiceDescriptors;
import com.cgi.eoss.osiris.rpc.ZooManagerServiceGrpc;
...
public class ZooManagerClient {
    ...
    public ZooManagerClient(DiscoveryClient discoveryClient,
                            @Value("${osiris.orchestrator.zooManager.eurekaServiceId:osiris zoo manager}") String zooManagerServiceId) {
     ...
     */
    public void updateActiveZooServices(List<OsirisService> services) {
        ...
        services.stream().map(this::convertToRpcOsirisService).forEach(argsBuilder::addServices);

        client.updateActiveZooServices(argsBuilder.build());
    }

    private WpsServiceDescriptor convertToRpcOsirisService(OsirisService osirisService) {
        return WpsServiceDescriptor.newBuilder()
                .setName(osirisService.getName())
                .setContent(ByteString.copyFromUtf8(osirisService.getServiceDescriptor().toYaml()))
                .build();
    }
