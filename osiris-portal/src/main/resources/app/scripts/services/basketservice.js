...
define(['../osirismodules', 'traversonHal'], function(osirismodules, TraversonJsonHalAdapter) {
    ...
    osirismodules.service('BasketService', ['$rootScope', '$http', 'osirisProperties', '$q', '$timeout', 'MessageService', 'UserService', 'TabService', 'CommunityService', 'FileService', 'traverson', function($rootScope, $http, osirisProperties, $q, $timeout, MessageService, UserService, TabService, CommunityService, FileService, traverson) {
        ...
        this.getDatabasketContents = function(databasket) {
            ...
            halAPI.from(rootUri + '/databaskets/' + databasket.id)
                ...
                .follow('files')
                .withRequestOptions({
                    qs: {projection: 'detailedOsirisFile'}
                })
                .getResource()
                .result
                .then(
        ...
        function getBasketItemLink(item, itemLinks) {
            ...
            if (item._links && item._links.self) {
                ...
                FileService.createGeoResultFile(item).then(function(osirisFile) {
                    itemLinks.push(osirisFile._links.self.href);
                    partialPromise.resolve();
                }, function(error) {
