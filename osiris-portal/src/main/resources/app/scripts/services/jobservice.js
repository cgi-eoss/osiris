...
define(['../osirismodules', 'traversonHal'], function (osirismodules, TraversonJsonHalAdapter) {
    ...
    osirismodules.service('JobService', [ 'osirisProperties', '$q', '$timeout', '$rootScope', 'MessageService', 'CommonService', 'UserService', 'CommunityService', 'traverson', function (osirisProperties, $q, $timeout, $rootScope, MessageService, CommonService, UserService, CommunityService, traverson) {
        ...
        var getJobOutput = function(job, outputLink) {
            ...
            halAPI.from(outputLink + '?projection=detailedOsirisFile')
                .newRequest()
                .getResource()
                .result
                .then(function (document) {
        ...
        function getOutputFiles(job){
            ...
            halAPI.from(rootUri + '/jobs/' + job.id + '/outputFiles?projection=detailedOsirisFile')
                .newRequest()
                .getResource()
                .result
                .then(function (document) {
                    ...
                    getJobDetails(job).then(function (details) {
                        ...
                        if (job.outputs) {
                            getOutputFiles(job).then(function(result){
                                job.outputFiles = result._embedded.osirisFiles;
