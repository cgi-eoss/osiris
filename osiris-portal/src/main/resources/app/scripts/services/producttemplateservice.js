...
define(['../osirismodules', 'traversonHal'], function (osirismodules, TraversonJsonHalAdapter) {
    ...
    osirismodules.service('ProductTemplateService', ['$rootScope', 'CommunityService', 'UserService', 'MessageService', '$http', 'osirisProperties', '$q', 'traverson', '$timeout', function ( $rootScope, CommunityService, UserService, MessageService, $http, osirisProperties, $q, traverson, $timeout) {
        ...
        this.getTemplateDetails = function(template) {
            ...
            halAPI.from(baseUrl + '/' + template.id + '?projection=detailedOsirisServiceTemplate')
                       .newRequest()
                       .getResource()
                       .result
                       .then(
        ...
        this.createService = function(data){
            return $q(function(resolve, reject) {
                  var service = {
                          ...
                          dockerTag: 'osiris/' + data.name.toLowerCase(),
                          serviceDescriptor: {
                            description: data.description,
                            id: data.name,
                            title: data.title,
