...
define(['../osirismodules', 'traversonHal'], function (osirismodules, TraversonJsonHalAdapter) {
    ...
    osirismodules.service('ProductService', ['$rootScope', 'CommunityService', 'UserService', 'MessageService', '$http', 'osirisProperties', '$q', 'traverson', '$timeout', function ( $rootScope, CommunityService, UserService, MessageService, $http, osirisProperties, $q, traverson, $timeout) {
        ...
        this.getService = function(service){
            var deferred = $q.defer();
            halAPI.from(rootUri + '/services/' + service.id + '?projection=detailedOsirisService')
                       .newRequest()
                       .getResource()
                       .result
                       .then(
