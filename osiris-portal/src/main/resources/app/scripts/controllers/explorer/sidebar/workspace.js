/**
 * @ngdoc function
 * @name osirisApp.controller:WorkspaceCtrl
 * @description
 * # WorkspaceCtrl
...
define(['../../../osirismodules'], function (osirismodules) {
    ...
    osirismodules.controller('WorkspaceCtrl', [ '$scope', 'JobService', 'SystematicService', 'ProductService', 'SearchService', 'MapService', 'CommonService', function ($scope, JobService, SystematicService, ProductService, SearchService, MapService, CommonService) {
        ...
        $scope.onDrop = function(dropObject, fieldId) {
            ...
            if(dropObject && dropObject.type === 'outputs') {
                for(var i = 0; i < dropObject.selectedOutputs.length; i++){
                    file = {
                        name: dropObject.selectedOutputs[i]._links.osiris.href,
                        link: dropObject.selectedOutputs[i]._links.osiris.href,
                ...
                for(var j = 0; j < dropObject.selectedItems.length; j++){
                    file = {
                        name: dropObject.selectedItems[j].properties._links.osiris.href,
                        link: dropObject.selectedItems[j].properties._links.osiris.href,
                        start: dropObject.selectedItems[j].properties.extraParams.osirisStartTime,
                        stop: dropObject.selectedItems[j].properties.extraParams.osirisEndTime,
                ...
                file = {
                    ...
                    link: dropObject.basket._links.osiris.href
                };
                if($scope.serviceParams.inputValues[fieldId] === undefined || $scope.serviceParams.inputValues[fieldId].indexOf(file.link) < 0){
                    $scope.serviceParams.dropLists[fieldId].push(file);
                }
                ...
                for(var k = 0; k < dropObject.selectedItems.length; k++) {
                    file = {
                        name: dropObject.selectedItems[k]._links.osiris.href,
                        link: dropObject.selectedItems[k]._links.osiris.href
                    };
                    if($scope.serviceParams.inputValues[fieldId] === undefined || $scope.serviceParams.inputValues[fieldId].indexOf(file.link) < 0){
                        $scope.serviceParams.dropLists[fieldId].push(file);
                    }
