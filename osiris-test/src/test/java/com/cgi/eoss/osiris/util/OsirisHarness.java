...
public class OsirisHarness {
    ...
    public void preScenario(Scenario scenario) {
        ...
        LOG.info("*** OSIRIS Services exposed on: {} ***", getOsirisBaseUrl());
        ...
        getChromeWebDriver().get(getOsirisBaseUrl() + "/app/");
    ...
    String getOsirisBaseUrl() {
        return "http://" +
                hostIp + ":" +
                environment.getServicePort("osiris-portal_1", 80);
    }
    ...
    private void containersAreUp() {
        ImmutableMap.of(
            ...
            Request webRoot = new Request.Builder().url(getOsirisBaseUrl() + path).build();
