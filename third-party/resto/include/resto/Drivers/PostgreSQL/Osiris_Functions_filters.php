 ...
 */
class Osiris_Functions_filters {
    ...
    public function prepareFilters($user, $model, $params) {
        ...
        $filterOsiris = $this->prepareOsirisFilter();
        $filters[] = $filterOsiris;
        return $filters;

    }
    protected function prepareOsirisFilter( ){
        $USER_HEADERS = array('REMOTE' , $this->_config['custom']['header'] );

        $user = null;
        foreach($USER_HEADERS as $h){
        ...
        if( null === $user ){
            RestoLogUtil::httpError(4004,'Userid not specified.');
        }

        $headers = array('REMOTE_USER' => $user );
        $opts = array('http' =>
            array(
                'method'  => 'GET',
        ...
        $uri = $this->_config['custom']['acl_server'];

        $context  = stream_context_create($opts);
        $result = file_get_contents($uri, false, $context );
        if($result===false){
            RestoLogUtil::httpError(500,'Cannot get user ACLs.');
        }

        $jsonObject = new JsonObject($result);

        $oids = $jsonObject->{$this->_config['custom']['path']};

        if($oids===false){
            RestoLogUtil::message(200,'Nothing matches.');
            return 'FALSE';
        }

        return 'identifier in ( '.implode(',',  array_map( function($e){ return  sprintf("'%s'", $e); }, $oids)).')';
    }
    ...
    private function prepareFilterQuery_contextualSearch($user, $model) {
        ...
        return $model->properties['visibility']['name'] . ' IN (' . implode(',', $visibilities) . ')';
