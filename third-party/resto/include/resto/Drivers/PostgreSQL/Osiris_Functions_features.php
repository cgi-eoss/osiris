 ...
 * under the License.
 */
require_once __DIR__.'/Functions_features.php';

/**
 * RESTo PostgreSQL features functions
 */
class Osiris_Functions_features  {
     ...
     * @throws Exception
     */
    public function __construct($dbDriver, $config) {
    ...
    public function search($context, $user, $collection, $params, $options) {
         ...
         */
        $filtersUtils = new Osiris_Functions_filters($this->_config);
        /*
         * Set model
         */
        ...
        //die("<pre><OSIRIS STUFF>".var_export($oFilter,true));

        /*
         * Prepare query
         */
        $fields = implode(',', $filtersUtils->getSQLFields($model));
        $from = ' FROM ' . (isset($collection) ? '_' . strtolower($collection->name) : $this->dbDriver->schemaName) .
            '.features' . ($oFilter ? ' WHERE ' . $oFilter : '');
