...
public class IptSearchProvider extends RestoSearchProvider {
    ...
    protected SearchResults postProcess(SearchResults results) {
        results.getFeatures().forEach(f -> addOsirisProperties(f, results.getParameters()));
        return results;
    }

    @SuppressWarnings("unchecked")
    private void addOsirisProperties(Feature feature, SearchParameters parameters) {
        ...
        URI osirisUri = externalProductService.getUri(productSource, productIdentifier);
        ...
        Optional.ofNullable(extraParams.get("cloudCover"))
                .ifPresent(cloudCoverage -> extraParams.put("osirisCloudCoverage", cloudCoverage));
        Optional.ofNullable(extraParams.get("orbitDirection"))
                .ifPresent(orbitDirection -> extraParams.put("osirisOrbitDirection", orbitDirection));
        Optional.ofNullable(extraParams.get("productType"))
                .ifPresent(productType -> extraParams.put("osirisProductType", productType));
        Optional.ofNullable(extraParams.get("published"))
        .ifPresent(published -> extraParams.put("osirisUpdated", published));
