...
import com.cgi.eoss.osiris.search.clms.model.CLMSCollection;
import com.cgi.eoss.osiris.search.clms.model.CLMSProduct;
import com.cgi.eoss.osiris.search.clms.model.CLMSResult;
import com.cgi.eoss.osiris.search.clms.model.Feed;
...
public class CLMSSearchProvider implements SearchProvider {
    ...
    private static final Set<String> INTERNAL_OSIRIS_PARAMS = ImmutableSet.of(
            "catalogue",
            "mission",
            "platform"
    );
	...
	private SearchResults postProcess(SearchResults results) {
		results.getFeatures().forEach(f -> {
			try {
				addOsirisProperties(f, results.getParameters());
	...
	private void addOsirisProperties(Feature feature, SearchParameters parameters) throws ParseException {
        ...
        URI osirisUri = builder.build().toUri();
        ...
        // These are added to extraParams so that the OsirisFile/Resto schema is predictable
        Map<String, Object> targetProperties = new HashMap<>();
        
        Optional.ofNullable(sourceProperties.get("beginPosition"))
        .ifPresent(startDate -> targetProperties.put("osirisStartTime", startDate));
        Optional.ofNullable(sourceProperties.get("endPosition"))
        .ifPresent(startDate -> targetProperties.put("osirisEndTime", startDate));
    ...
    public Map<String, String> getQueryParameters(SearchParameters parameters) {
        ...
        parameters.getParameters().asMap().entrySet().stream()
                .filter(p -> !INTERNAL_OSIRIS_PARAMS.contains(p.getKey()) && !p.getValue().isEmpty() && !Strings.isNullOrEmpty(Iterables.getFirst(p.getValue(), null)))
