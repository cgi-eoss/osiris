...
import com.cgi.eoss.osiris.model.projections.DetailedDatabasket;
import com.cgi.eoss.osiris.model.projections.ShortDatabasket;
...
public class DatabasketResourceProcessor extends BaseResourceProcessor<Databasket> {
    ...
    private void addOsirisLink(Resource resource, Long databasketId) {
        resource.add(new Link(CatalogueUri.DATABASKET.build(ImmutableMap.of("id", String.valueOf(databasketId))).toASCIIString()).withRel("osiris"));
    }
    ...
    private final class BaseEntityProcessor implements ResourceProcessor<Resource<Databasket>> {
        ...
        public Resource<Databasket> process(Resource<Databasket> resource) {
            ...
            addOsirisLink(resource, entity.getId());
