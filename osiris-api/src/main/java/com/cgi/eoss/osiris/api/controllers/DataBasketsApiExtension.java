...
import com.cgi.eoss.osiris.search.api.SearchResults.Page;
...
public class DataBasketsApiExtension {
    ...
    public DataBasketsApiExtension(SearchApi searchApi, CatalogueService catalogueService, DatabasketDataService databasketDataService,
            @Value("${osiris.api.maxItemsDatabasketsFromSearch:500}") int maxItemsDatabasketsFromSearch) {
    ...
    private void addSearchResultsToDatabasket(Databasket databasket, SearchResults searchResults) {
        ...
        for (Feature feature : features) {
            ...
            Map<String, Object> osirisLink = (Map<String, Object>) links.get("osiris");
            String osirisLinkHref = (String) osirisLink.get("href");
            ...
            newFile.setProperty("originalUrl", osirisLinkHref);
            newFile.setProperty("extraParams", feature.getProperty("extraParams"));
            OsirisFile osirisFile = catalogueService.indexExternalProduct(newFile);
            LOG.debug("Adding osirisFile {} to databasket {}", newFile.getId(), databasket.getId());
            databasket.getFiles().add(osirisFile);
        }
    }
